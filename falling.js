...
const gameCoinsound = new Audio('../assets/game-start-317318.mp3')
gameOverSound.volume = 0.3;
gameLoopsound.volume = 0.2;
gameCoinsound.volume = 0.5;
...
const equippedMap = getItemById(currentUserData?.equippedMap_easy || '../assets/Firewatch.png');
...
const totalFrames = 3;
const frameWidth = 305 / totalFrames;
...
function showGameOver(coinsEarned, bonusEarned) {  
  ...
  const previousHigh = currentUserData?.scores?.easy || 0;
  if (score >= previousHigh) {
    currentUserData.scores.easy = score;
...
function animatePlayer() {
  ...
  player.style.backgroundImage = `url(${playerImg})`;
...
function updateGame(timestamp) {
  ...
  meteors.forEach((m, index) => {
    ...
    // Upon user death these coins will be added to the localStorage value coins while also changing and storing the currentUserData.score.hard high score      const playerY = window.innerHeight * 0.8;
    if (
      ...
      gameCoinsound.currentTime = 0;
      gameCoinsound.play();
      if (currentUserData) {
        if (!currentUserData.scores) currentUserData.scores = {};
        if (!currentUserData.scores.easy) currentUserData.scores.easy = 0;
        currentUserData.coins += coinsEarned + bonusCoins;
        if (score > currentUserData.scores.easy) {
          currentUserData.scores.easy = score;
        }
        localStorage.setItem('user_' + currentuserName, JSON.stringify(currentUserData));
      }
  ...
  if (Math.random() < 0.015) createMeteor(-40);
